...
matching? Equivalently: a red/blue colouring using both colours in
which every vertex has at most one neighbour of the opposite colour
("budget ≤ 1 cross edge per vertex"). We use the colouring form
...
Membership in NP is trivial (the colouring is a certificate). Hardness
...
- §4: structure of valid colourings — WLOG u red; case split A1/A0/B;
...
EXACTLY ONE L1-neighbour each (w_a; q_i; q_i; w_p). G_W has exactly two
...
to u (distance 1). Every L2 vertex has an L1-neighbour (witness counts
above), so is at distance 2 (it is not adjacent to u: u's neighbours
are exactly L1). Every L3 vertex has an L2-neighbour (g_i ~ a; c ~ p;
z_C ~ p_{C,1}; Zk ~ T1) and no shorter connection (its neighbours are
...
neighbours are exactly L1), so dist(v,u) ≥ 3.
...
all of L2: every L1 vertex has at most 3 L2-neighbours (inspect §2:
...
L1-neighbours (witness counts above), while |L1| ≥ 5 (w_a and the four
...
## 4. Valid colourings: WLOG and case analysis

Fix any valid colouring χ (both colours used, every vertex ≤ 1 cross).

**WLOG u is red.** The colour swap R↔B is an involution on valid
colourings (the definition is colour-symmetric), so a valid colouring
...
⊆ L3 (u, L1, L2 are all red), and blue ≠ ∅ (both colours used). A blue
z ∈ L3 has all its neighbours in L2 (§2), all red, so it carries
...
L1-neighbours and both are red, then v is red: blue v would cross both
witnesses (2 > 1). ∎
...
other neighbour of b is blue. By Claim 4.3, every L2 vertex whose two
...
1. b = W ∈ BW (a base witness q_i, w_a, or w_p). G_W ~ b, so G_W is
   ...
   w'_W consumes G_W's budget, so G_W's remaining neighbour Zk is
   blue. Zk's neighbours are T1, T2, and every guard: T1, T2 are red
...
3. b ∈ {wT1a, wT1b} (T2 symmetric). T1 ~ b is blue. T1's other
   ...
   remaining neighbour Zk is blue. Zk's neighbours T2 (red: its
...
All branches are contradictory, so case B admits no valid colouring. ∎
...
w : S → L1 map v to its unique L1-neighbour.
...
L1-neighbour (a BFS-layer-2 vertex has a neighbour at distance 1 from
u by definition), so "budget forces exactly one L1-neighbour" in
...
Proof. (⇒) Let χ be such a colouring; put B2 := blue ∩ L2 ≠ ∅.
- Every v ∈ B2 has all its L1-neighbours red (hypothesis), each
  contributing a cross at v; budget forces exactly one L1-neighbour,
  so B2 ⊆ S, and v's budget is then exhausted, so every L3-neighbour
  of v is blue. Hence every z ∈ N(B2) ∩ L3 is blue.
- (α) A witness W ∈ L1 is red; each blue L2-neighbour of W is a cross
  at W; budget ≤ 1. Two B2-members sharing W would give 2.
- (β') A blue z ∈ L3 has crosses exactly at its red L2-neighbours
  (N(z) ⊆ L2); these include ALL of N(z) ∖ B2 (any L2 vertex outside
  B2 is red, whether selectable or not); budget ≤ 1.
- (γ') A red v ∈ L2 ∖ B2 has a cross at each blue L3-neighbour; the
  vertices of N(v) ∩ N(B2) ∩ L3 are all blue (first bullet);
  budget ≤ 1.
(⇐) Given such a B2, colour blue exactly B2 ∪ (N(B2) ∩ L3), red
everything else. Check every vertex's budget:
- u: all neighbours (L1) red; 0 crosses.
- W ∈ L1: red; crosses = blue L2-neighbours = |N(W) ∩ B2| ≤ 1 by (α)
  (each B2-member has W as its unique witness, so two would share W).
- v ∈ B2: blue; crosses = red neighbours = the unique witness (red)
  plus red L3-neighbours; but N(v) ∩ L3 ⊆ N(B2) ∩ L3 is entirely
  blue; total 1.
- z ∈ N(B2) ∩ L3: blue; crosses = red L2-neighbours = |N(z) ∖ B2| ≤ 1
  by (β').
- v ∈ L2 ∖ B2: red; its neighbours are in L1 (red) and L3; blue
  L3-neighbours are exactly N(v) ∩ N(B2) ∩ L3 (nothing else in L3 is
  blue); ≤ 1 by (γ').
- z ∈ L3 ∖ N(B2): red; its neighbours are in L2; blue ones would be
  in B2 ∩ N(z) = ∅ (z ∉ N(B2)); 0 crosses.
Both colours are used (u red, B2 ≠ ∅ blue). ∎
...
per group, with (β) every z ∈ N(B) having ≤ 1 neighbour outside B and
(γ) every x ∈ X ∖ B having ≤ 1 neighbour in N(B)? (Here neighbourhoods
...
w_p serves p; note G_{q_i} is ALSO a neighbour of q_i but is not in S,
...
- (β' = β): the L3 vertices reachable from B2 ⊆ S are base vertices
  only (Zk's L2-neighbours are guards and ballast, none in S), and
  every base L3 vertex has ALL its L2-neighbours in S (g_i ~ a,t_i,f_i;
  c ~ base, proxy; z_C ~ proxies — §2), so "N(z) ∖ B2 over all L2" =
  "N(z) ∖ B over X".
- (γ' = γ): for v ∈ S ∖ B2 the two conditions are verbatim identical
  (v's L3-neighbourhood consists of base vertices). For v ∈ L2 ∖ S
  (guards, ballast): N(v) ∩ L3 = {Zk}, and Zk ∉ N(B2) as above, so
  (γ') holds vacuously at v.
Hence: G(F) has a case-A1 colouring iff ISLAND(I(F)) is YES. ∎
...
1. Selected proxy ⇒ its base selected: p ∈ B touches both copies
   ...
   outside B with TWO touched neighbours — (γ) violated.
2. Selected base ⇒ a selected: t_i ∈ B touches g_i; f_i ∉ B by (α);
   if a ∉ B then g_i has two neighbours outside B — (β) violated.
...
5. Selected base ⇒ its proxies selected: base ∈ B touches both copies
   ...
   occurrence would have two touched neighbours — (γ). With step 1:
...
matching cut iff a case-A1 colouring exists (§4: WLOG u red; A0 dies
...
- Independent oracle (independent_mc.py, plain exhaustive backtracking,
  ...
  0 mismatches; complete enumeration of every valid colouring of a
...
- Second review (scratch/review-block10-scripts/), all fresh code:
  ...
  valid-colouring enumeration of six guarded graphs to |V|=62 (all
  colourings are A1; UNSAT instances have none); fresh-seed e2e 163
